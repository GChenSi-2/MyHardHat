...
export type { Counter, CounterInterface } from "../../../types/ethers-contracts/Counter";
export type { HelloWorld, HelloWorldInterface } from "../../../types/ethers-contracts/HelloWorld";
...
const deployments = Object.entries(deploymentsJson as DeploymentJson).reduce<DeploymentMap>(
  (acc, [chainId, contracts]) => {
    const numericChainId = Number.parseInt(chainId, 10);
    acc[numericChainId] = Object.entries(contracts).reduce<Partial<Record<ContractName, Address>>>(
...
export const DEFAULT_CHAIN_ID = Number.parseInt(process.env.NEXT_PUBLIC_CHAIN_ID ?? "31337", 10);
...
export function getSupportedChains(): number[] {
  return Object.keys(deployments).map((chainId) => Number.parseInt(chainId, 10));
}
